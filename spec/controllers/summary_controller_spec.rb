...
describe SummaryController do
  ...
  describe "GET #edit signing in" do
    ...
    it "edit content" do
      ...
      assigns[:summary_num].should == 1
      assigns[:content].should == "10年以上auの携帯電話を使っている筆者は昨年12月、auのiPhone 5に切り替えた。補償するという意味合いが強かった。KDDIは公式的には消費者への補償はしないと説明していたのに、実際の現場では強くクレームを付ける消費者には補償対応している。まさに「二枚舌営業」なのである。今度は取材として広報部に「二枚舌ではないか」と聞くと、広報部長は「公式見解と個別の顧客対応は違う」と説明、「二枚舌営業」を自ら認めた。大株主のトヨタには特別対応?。これらの疑問についてKDDI広報に聞くと、トヨタへは「特別対応」しているという。トヨタはKDDIの大株主であり、業務用携帯はauを使っているからであろう。"
      assigns[:content_num].should == 299
    end
    it "edit no content" do
      get :edit, :article_id => "69"
      response.should be_success
      assigns[:url].should == "http://www.amazon.com/gp/product/B000NQRE9Q/ref=s9_qpp_gw_d99_g74_ir03?pf_rd_m=ATVPDKIKX0DER&pf_rd_s=center-5&pf_rd_r=1JS0Y4VZMR0PEDPPW98G&pf_rd_t=101&pf_rd_p=1630072322&pf_rd_i=507846"
      assigns[:title].should == "Amazon.com: Indiana Jones: The Complete Adventures [Blu-ray]"
      assigns[:summary_num].should == 1
